...
window.addEventListener('message', function (event) {
    ...
    if (event.data.action == 'SET_LEVELING_DISPLAY_STATUS') {
    ...
    } else if (event.data.action == 'SET_HUD_DISPLAY_HORSE_STATUS') {

        event.data.hasHorse ? $('#varHorseThirst').show() : $('#varHorseThirst').hide();
        event.data.hasHorse ? $('#varHorseHunger').show() : $('#varHorseHunger').hide();
        ...
        if (event.data.horse_hunger) {
            $("#varHorseHunger").attr("class", "c100 p" + Math.ceil(event.data.horse_hunger));
            setColouredBasedOnValue("varHorseHunger", event.data.horse_hunger);
        }

        if (event.data.horse_thirst) {
            $("#varHorseThirst").attr("class", "c100 p" + Math.ceil(event.data.horse_thirst));
            setColouredBasedOnValue("varHorseThirst", event.data.horse_thirst);
        }
        ...
        if (event.data.alcohol) {
            ...
            $("#varAlcohol").css("background-color", alcohol >= 50 ? "#750c0ca" : "#161616a8");
        }
        ...
        if (event.data.criminal) {
            $("#varCriminal small").text(Math.ceil(event.data.criminal.level));
            $("#varCriminal").attr("class", "c100 small first p" + Math.trunc(event.data.criminal.experience));
        }
    ...
    function setColouredBasedOnValue(elementId, value) {
        ...
        if (value == -1 || value <= 0 || elementId == "varDirt" && value <= 25 || elementId == "varStress" && value <= 25 || elementId == "varAlcohol" && value <= 25) {
